...
// details: https://www.wrensoft.com/zoom/usersguide.html
//
// IF YOU NEED TO ADD PHP TO THE SEARCH PAGE, see this FAQ:
// https://www.wrensoft.com/zoom/support/faq_ssi.html
//
// zoom@wrensoft.com
// https://www.wrensoft.com
// ----------------------------------------------------------------------------
...
if (isset($_POST['zoom_per_page']))
{
	$per_page = intval($_POST['zoom_per_page']);
	if ($per_page < 1)
		$per_page = 1;
}
else
...
$NewSearch = 0;
if (isset($_POST['zoom_page']))
{
	$page = intval($_POST['zoom_page']);
	if ($page < 1)
		$page = 1;
}
else
...
$IsZoomAC = 0;
if (isset($_POST['zoom_ac']))
{
	$IsZoomAC = intval($_POST['zoom_ac']);
	$ZoomACOutput = "";
	if ($IsZoomAC == 1)
		$SearchAsSubstring = 1;
}
...
if (isset($_POST['zoom_and']))
	$and = intval($_POST['zoom_and']);
...
if ($UseCats == 1)
{
	if (isset($_POST['zoom_cat']))
	{
		if (is_array($_POST['zoom_cat']))
			$cat = $_POST['zoom_cat'];
		else
			$cat = array($_POST['zoom_cat']);
...
if (isset($_POST['zoom_sort']))
	$sort = intval($_POST['zoom_sort']);
else
	$sort = $DefaultSort;
...
{
	if (isset($_POST['zoom_datefrom']))
		$date_from = $_POST['zoom_datefrom'];
	else
		$date_from = "";
	if (isset($_POST['zoom_dateto']))
		$date_to = $_POST['zoom_dateto'];
	else
		$date_to = "";
}
...
function ShowTemplate()
{
	...
	// for a solution: https://www.wrensoft.com/zoom/support/faq_ssi.html
...
if ($UseMetaFields == 1)
{
	...
	{
		if (isset($_POST[$metafields[$fieldnum][$METAFIELD_NAME]]))
		{
			$meta_query[$fieldnum] = $_POST[$metafields[$fieldnum][$METAFIELD_NAME]];
...
{
	// Insert the form
	$OutputBuffers[$OUTPUT_FORM_SEARCHBOX] = "<label for=\"zoom_searchbox\">$STR_FORM_SEARCHFOR</label><br />	\n";
...
if (empty($query) && $IsZoomAC == 0)
{
	...
	if ($NoSearch)
	{
		...
		if ($ZoomInfo == 1)
			$OutputBuffers[$OUTPUT_PAGENUMBERS] .= "<p style=\"text-align: center;\"><small>" . $STR_POWEREDBY . " <a href=\"https://www.wrensoft.com/zoom/\" target=\"_blank\"><b>Zoom Search Engine</b></a></small></p>";
...
$OutputBuffers[$OUTPUT_HEADING] .= "<div class=\"searchheading\">" ;
if ($UseCats)
{
	if ($cat[0] == -1)
	{
...
// published on Martin Porter's website (https://tartarus.org/~martin/PorterStemmer/)
// ----------------------------------------------------------------------------------
class PorterStemmer
{
